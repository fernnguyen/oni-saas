-- migration: 20260630000006_subscription_pastdue_readonly.sql
-- Block write permissions if subscription is past_due
...
begin
  ...
  if p_permission not like '%view%' and p_permission not like 'billing.%' then
    ...
    -- Only block if explicitly deleted, locked, or past_due
    -- past_due means they are in the grace period, they cannot write/trade, only read/export
    if v_sub_status in ('deleted', 'locked', 'past_due') then
      return false;
    end if;
  end if;

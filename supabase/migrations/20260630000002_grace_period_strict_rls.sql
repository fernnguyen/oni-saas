-- migration: 20260630000002_grace_period_strict_rls
-- Strictly restrict write permissions by directly checking current_period_end instead of waiting for cron
...
begin
  ...
  if p_permission not like '%view%' and p_permission not like 'billing.%' then
    select status, current_period_end into v_sub from public.subscriptions where tenant_id = p_tenant_id limit 1;
    if v_sub.status in ('past_due', 'locked', 'deleted') or v_sub.current_period_end < now() then
      return false;
    end if;
  end if;

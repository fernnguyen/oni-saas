    ...
    <div className="flex items-center gap-2 justify-end">
      ...
      {open && typeof document !== 'undefined' && createPortal(
        <div className="row-actions-dropdown fixed z-[9999] w-48 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl ring-1 ring-black/5" style={{ top: coords.top, right: window.innerWidth - coords.left - coords.width }}>
          <button onClick={() => { setOpen(false); onEdit() }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 transition-colors">
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col gap-0.5">
          {row.sku ? <CopyableId id={cleanSku(row.sku)} className="text-[11px] font-semibold text-slate-500 w-fit" /> : <span className="text-[11px] text-slate-400">—</span>}

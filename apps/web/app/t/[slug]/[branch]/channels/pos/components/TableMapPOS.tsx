...
export function TableMapPOS({
  ...
  const selectedOrders = useMemo(() => {
    return selectedResourceIds.map(resId => {
      const order = ordersMap.get(`res-${resId}`) || ordersMap.get(resId) || inProgressOrders.find(o => {
        try {
  ...
  function handleResourceClick(r: Resource) {
    if (groupCheckoutMode) {
      if (r.status !== 'occupied') {
        toast.info('Chỉ có thể chọn phòng/bàn đang có khách sử dụng để thanh toán nhóm')
        return
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-88px)] md:h-[calc(100vh-104px)] gap-4 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          >
            ...
            <span>{groupCheckoutMode ? 'Hủy chọn nhóm' : 'Thanh toán nhóm'}</span>
          </button>
                ...
                {items.sort((a: Resource, b: Resource) => Number(a.sort_order || 0) - Number(b.sort_order || 0)).map((r: Resource) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={groupCheckoutMode && !isOccupied}
                      className={`group relative rounded-2xl border p-4 flex flex-col text-left transition-all hover:shadow-lg cursor-pointer overflow-hidden ${
                        groupCheckoutMode && !isOccupied ? 'opacity-40 cursor-not-allowed' : ''
                      } ${
      ...
      {/* Floating Control Bar for Group Checkout */}
      {groupCheckoutMode && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 bg-white/95 backdrop-blur-md border border-slate-200 px-6 py-4 rounded-3xl shadow-xl flex items-center gap-6 animate-in slide-in-from-bottom duration-300">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-slate-450 uppercase tracking-wider">Thanh toán nhóm</span>
            <span className="text-sm font-black text-slate-800">Đã chọn: <span className="text-primary font-black">{selectedResourceIds.length}</span> vị trí</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
              ...
              className="px-4 py-2 text-xs font-bold text-slate-650 hover:bg-slate-100 rounded-xl transition-all cursor-pointer"
            >
              Hủy
            </button>
            <button
              disabled={selectedResourceIds.length === 0}
              onClick={() => setGroupCheckoutModalOpen(true)}

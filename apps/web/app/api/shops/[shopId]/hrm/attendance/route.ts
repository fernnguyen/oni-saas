...
const updateAttendanceDaySchema = z.object({
  ...
  clock_in_source: z.string().nullable().optional(),
  clock_in_metadata: z.any().nullable().optional(),
  clock_out_source: z.string().nullable().optional(),
  clock_out_metadata: z.any().nullable().optional(),
  ...
  try {
    ...
    if (input.action === 'check_in') {
      await access.repository.clockIn({
        ...
        source: input.source || (employeeId === selfEmployeeId ? 'self' : 'manual_by_manager'),
...
export async function PUT(
  ...
  try {
    ...
    await access.repository.updateAttendanceDay({
      ...
      clockInSource: input.clock_in_source,
      clockInMetadata: input.clock_in_metadata,
      clockOut: input.clock_out,
      clockOutSource: input.clock_out_source,
      clockOutMetadata: input.clock_out_metadata,

...
export function PaymentStatusLabel({ status, amount, className }: Props) {
  ...
  return (
    <div className={['inline-flex items-center gap-2 rounded-md px-2 py-1', styles.bg, className].filter(Boolean).join(' ')}>
      {styles.icon}
      <span className="text-xs font-medium">
        {formatAmount(amount)} <span className="border-b border-dashed border-current">đ</span>
      </span>
    </div>
  )
}

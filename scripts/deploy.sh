...
// Doc shared/.env khi PM2 khoi dong -- ho tro multiline values
function loadEnv() {
  const base = { PORT: APP_PORT, NODE_ENV: 'production', HOSTNAME: '0.0.0.0' };
  if (!fs.existsSync(SHARED_ENV)) return base;
  const raw = fs.readFileSync(SHARED_ENV, 'utf8');
  const env = Object.assign({}, base);
  let i = 0;
  while (i < raw.length) {
    const nl   = raw.indexOf('\n', i);
    const line = (nl === -1 ? raw.slice(i) : raw.slice(i, nl)).trimEnd();
    ...
    if (val[0] === '"' || val[0] === "'") {
      ...
      let closed = false;
      while (!closed) {
        const ci = val.indexOf(q);
        if (ci !== -1) { val = val.slice(0, ci); closed = true; }
        else {
          const nni  = raw.indexOf('\n', i);
          const next = nni === -1 ? raw.slice(i) : raw.slice(i, nni);
          i = nni === -1 ? raw.length : nni + 1;
          val += '\n' + next;
        }
      }
    }
...
ECOEOF
echo "   OK ecosystem.config.js generated (reads shared/.env at PM2 start)"

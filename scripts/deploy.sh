...
const sharedEnv = process.env.SHARED_DIR + '/.env';
const standalone = process.env.STANDALONE;
const releaseDir = process.env.RELEASE_DIR;
const version    = process.env.VERSION;
const appPort    = process.env.APP_PORT || '3000';

// Parse shared/.env — không dùng dotenv để zero dependencies
const env = { PORT: appPort, NODE_ENV: 'production', HOSTNAME: '0.0.0.0' };
if (fs.existsSync(sharedEnv)) {
  const lines = fs.readFileSync(sharedEnv, 'utf8').split('\n');
  ...
  apps: [
    {
      ...
      listen_timeout: 8000,
      kill_timeout: 5000,
      env: ${JSON.stringify(env, null, 8).replace(/^/gm, '      ').trim()},
    },
  ],
};
`;

fs.writeFileSync(releaseDir + '/ecosystem.config.js', content);
console.log('   ✅ ecosystem.config.js generated with', Object.keys(env).length, 'env vars injected');
NODESCRIPT
